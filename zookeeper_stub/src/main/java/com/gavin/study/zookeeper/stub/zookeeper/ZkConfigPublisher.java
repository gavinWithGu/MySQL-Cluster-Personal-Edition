...
import com.gavin.study.zookeeper.stub.utils.ZkUtils;
...
public class ZkConfigPublisher {
	...
	public static String ZK_CONFIG_ROOTNODE = "/gavin/config/sampleConfig";
	...
	private static final void loadProperties() {
		...
		if (is == null) {
			throw new RuntimeException("找不到system.properties资源文件.");
		}
		...
		ZK_CONFIG_ROOTNODE = props.getProperty("zk.configs.rootnode");
		ZK_CONF_ENCODING = props.getProperty("zk.configs.encoding");
		ZK_TIMEOUT = Integer.parseInt(props.getProperty("zk.client.timeout"));
		ZK_ADDRESS = props.getProperty("zk.host");
		CONF_DIR = props.getProperty("zk.conf.dir");
	}
